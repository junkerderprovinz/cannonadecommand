...
console.log('\nTHE KEY REGRESSION PIN: plugIconInk() must resolve from eff("iconbg"), never from the DOM class');
{
  ...
  ok('cc.iconbg=1 with the DOM class left UNSET still resolves the badge-mode ink', /^#[0-9a-f]{6}$/i.test(pluginsApi.plugIconInk(false)), pluginsApi.plugIconInk(false));
  ok('and it equals idealText() of the picked colour (the badge-mode branch), not the raw picked colour', pluginsApi.plugIconInk(false) === pluginsApi.idealText('#e5a00d'), pluginsApi.plugIconInk(false));
  reset();
